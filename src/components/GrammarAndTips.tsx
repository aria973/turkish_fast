...
import { brainMethods } from '../data/memoryHooks';
...
export const GrammarAndTips: React.FC<GrammarProps> = ({ soundEnabled }) => {
  const [selectedSection, setSelectedSection] = useState<'lego' | 'magic-words' | 'etiquette' | 'brain'>('lego');
  ...
  return (
    <div className="pb-24 pt-3 px-4 max-w-md mx-auto space-y-5">
      ...
      <div className="bg-gradient-to-br from-indigo-900 via-purple-900 to-red-900 text-white rounded-3xl p-5 shadow-xl border border-purple-800">
        ...
        <div className="grid grid-cols-4 gap-1.5 mt-4 bg-black/35 p-1 rounded-2xl border border-white/10">
          <button
            onClick={() => { triggerHaptic('light'); if (soundEnabled) playBeep('tap'); setSelectedSection('brain'); }}
            className={`py-2 px-1 rounded-xl text-[11px] font-bold transition-all tap-target ${
              selectedSection === 'brain' ? 'bg-purple-600 text-white shadow' : 'text-gray-300 hover:text-white'
            }`}
          >
            🧠 Brain
          </button>
      ...
      {selectedSection === 'brain' && (
        <div className="space-y-3">
          <div className="bg-purple-100 dark:bg-purple-950/40 p-4 rounded-2xl border border-purple-200 dark:border-purple-900 text-sm text-purple-950 dark:text-purple-200 font-medium leading-relaxed">
            <strong>Why this app is built the way it is.</strong> Every screen applies a specific finding from memory
            research. Understanding the method makes you use it properly — and trust it when a session feels hard.
          </div>

          {brainMethods.map((m) => (
            <div
              key={m.id}
              className="bg-white dark:bg-[#2c2c2e] p-4 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-800 space-y-2.5"
            >
              <h3 className="text-base font-black text-gray-900 dark:text-white flex items-center gap-2">
                <span className="text-xl">{m.icon}</span> {m.name}
              </h3>

              <p className="text-xs text-gray-600 dark:text-gray-300 leading-relaxed border-l-4 border-purple-400 pl-2.5">
                <strong className="text-purple-700 dark:text-purple-300">Research:</strong> {m.finding}
              </p>

              <p className="text-xs text-gray-800 dark:text-gray-200 leading-relaxed bg-gray-50 dark:bg-[#38383a] rounded-xl p-2.5">
                <strong>In this app:</strong> {m.howAppUses}
              </p>

              <p className="text-xs font-bold text-emerald-800 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/40 rounded-xl p-2.5 border border-emerald-200 dark:border-emerald-900/50">
                ✅ Do this: {m.doThis}
              </p>
            </div>
          ))}
