...
import { X, Heart, Check, ArrowRight, RotateCcw, Sparkles, Trophy, BookOpenText, ScrollText } from 'lucide-react';
...
export const SessionPlayer: React.FC<SessionPlayerProps> = ({
  part, type, soundEnabled, onExit, onComplete
}) => {
            ...
            >
              ...
              <div className="text-center space-y-3 my-4">
                {!revealed ? (
                  ...
                  <>
                    <h2 className="text-2xl font-black leading-tight">{step.item.turkish}</h2>
                    <p className="text-sm text-white/85 font-semibold">"{step.item.english}"</p>
                    <div className="bg-black/25 rounded-2xl px-3 py-2.5 mx-auto max-w-sm">
                      <p className="text-[10px] font-black uppercase tracking-wider text-amber-300 flex items-center justify-center gap-1 mb-1">
                        <ScrollText className="w-3 h-3" /> Say it out loud:
                      </p>
                      <p className="text-xs text-amber-100 font-mono font-bold leading-relaxed">{step.item.pronunciation}</p>
                    </div>
                  </>
                )}
              </div>
              {revealed && step.item.grammarTip && (
                <div className="bg-black/25 rounded-2xl p-3 text-[11px] leading-relaxed text-white/95">
            ...
            {step.kind === 'listen' ? (
              <div className="text-center space-y-3 py-2">
                ...
                <div className="bg-white dark:bg-[#2c2c2e] rounded-3xl border-2 border-red-200 dark:border-red-900/60 p-5 shadow-sm space-y-2">
                  <h2 className="text-xl font-black text-gray-900 dark:text-white leading-snug">{step.item.turkish}</h2>
                  <div className="bg-amber-50 dark:bg-amber-950/40 rounded-xl px-3 py-2">
                    <p className="text-[10px] font-black uppercase tracking-wider text-amber-600 dark:text-amber-400 mb-0.5">Transcript</p>
                    <p className="text-xs text-amber-900 dark:text-amber-200 font-mono font-bold leading-relaxed">{step.item.pronunciation}</p>
                  </div>
                </div>
              </div>
            ) : (
              ...
              {step.options.map((opt, i) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-4 rounded-2xl border-2 text-left text-sm font-bold transition-all active:scale-[0.98] ${style}`}
                  >
