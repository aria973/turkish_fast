...
import { SRSApi } from '../hooks/useSRS';
import { studyProtocol } from '../data/memoryHooks';
import { SessionType, PartDef, MasteryLevel } from '../types';
import { Flame, Trophy, Star, Lock, Check, Zap, RotateCcw, BookA, Sparkles, Brain } from 'lucide-react';
...
export const CourseMap: React.FC<CourseMapProps> = ({ progress, srs, soundEnabled, onOpenSession, onOpenReview }) => {
  ...
  return (
    <div className="pb-24 pt-3 px-4 max-w-md mx-auto space-y-5">
      ...
      <button
        ...
        className={`w-full rounded-3xl p-4 shadow-sm border text-left transition-all active:scale-[0.99] tap-target ${
          srs.dueCount > 0
            ? 'bg-gradient-to-r from-violet-600 to-fuchsia-700 text-white border-transparent shadow-lg'
      ...
      >
        <div className="flex items-center gap-3.5">
          <span className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${
            srs.dueCount > 0 ? 'bg-white/20' : 'bg-violet-100 dark:bg-violet-950/60'
          }`}>
            <Brain className={`w-6 h-6 ${srs.dueCount > 0 ? 'text-white' : 'text-violet-600 dark:text-violet-300'}`} />
          </span>
          <div className="flex-1 min-w-0">
            <p className={`text-sm font-black ${srs.dueCount > 0 ? 'text-white' : 'text-gray-900 dark:text-white'}`}>
              Smart Review {srs.dueCount > 0 && <span className="ml-1">· {srs.dueCount} due now</span>}
            </p>
            <p className={`text-xs font-semibold leading-snug mt-0.5 ${
              srs.dueCount > 0 ? 'text-white/85' : 'text-gray-500 dark:text-gray-400'
            }`}>
              {srs.dueCount > 0
                ? 'Recall these before you forget them — spacing beats cramming.'
                : srs.trackedCount === 0
                  ? 'Finish a Learn session to start your memory schedule.'
                  : `All caught up. ${srs.strongCount} phrases strong${srs.nextDueLabel ? ` · next in ${srs.nextDueLabel}` : ''}.`}
            </p>
          </div>
          {srs.dueCount > 0 && (
            <span className="bg-white text-violet-700 text-sm font-black px-3 py-1.5 rounded-xl shrink-0">
              {srs.dueCount}
            </span>
          )}
        </div>
      ...
      {/* ============ HOW TO STUDY (NEUROSCIENCE PROTOCOL) ============ */}
      <details className="bg-white dark:bg-[#2c2c2e] rounded-3xl border border-gray-200 dark:border-gray-800 overflow-hidden group">
        <summary className="p-4 cursor-pointer flex items-center justify-between tap-target">
          <span className="text-sm font-black text-gray-900 dark:text-white flex items-center gap-2">
            🧬 The 16-minute memory protocol
          </span>
          <span className="text-xs font-bold text-red-600 group-open:rotate-180 transition-transform">▼</span>
        </summary>
        <div className="px-4 pb-4 space-y-2">
          <p className="text-xs font-medium text-gray-600 dark:text-gray-300 leading-relaxed">
            Built on retrieval practice, spacing, dual coding and sleep consolidation research. Follow it in order.
          </p>
          {studyProtocol.map(s => (
            <div key={s.step} className="flex items-start gap-3 bg-gray-50 dark:bg-[#38383a] rounded-2xl p-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-extrabold text-gray-900 dark:text-white">
                  {s.title} <span className="text-xs font-bold text-gray-400">· {s.mins}</span>
                </p>
                <p className="text-xs font-medium text-gray-600 dark:text-gray-300 leading-snug">{s.detail}</p>
              </div>
            </div>
          ))}
        </div>
